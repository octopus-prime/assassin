...
/*
		...
		{
			_report(iteration, _height, score * node.color(), clock::now() - _start, _count, get_pv(node, iteration));
			score = search(node, -30000, score, iteration * PLY, 0, move_t {});
		}
		else if (score >= beta)
		{
			_report(iteration, _height, score * node.color(), clock::now() - _start, _count, get_pv(node, iteration));
			score = search(node, score, +30000, iteration * PLY, 0, move_t {});
		}
...
//			return evaluator::evaluate(node);;
	...
	const bool skip = entry && entry->depth >= depth - reduction && entry->score < search_beta && entry->flag == transposition_table_t::UPPER;
	...
	{
		search(node, search_alpha, search_beta, depth - 2 * PLY, height + 1, best_move);
		const transposition_table_t::entry_t* const entry = _t_table[node];
		if (entry)
			best_move = entry->move;
	}
...
//				_t_table.prefetch(succ);
				...
				const score_t score = -search(succ, -search_beta, -best_score, depth - 1 * PLY + ext, height + 1, best_move);
				if (score > best_score)
				{
					if (score >= search_beta)
					{
						_t_table.put(node, best_move, search_beta, transposition_table_t::LOWER, depth + ext);
	...
	{
	case 3:
		if (!check && scoreM + 1_Q <= best_score)
			depth -= PLY;
		break;
	case 2:
		scoreX = scoreM + 1_R;
	...
	tbb::filter_t<void, move_t> generating
	(
		tbb::filter::serial_in_order,
		[&](tbb::flow_control& fc)
		{
			if (!tbb::task::self().is_cancelled() && iterator != moves.end())
				return *iterator++;
			fc.stop();
			return move_t {};
		}
	);
	typedef std::pair<move_t,score_t> move_score_t;
	tbb::filter_t<move_t, move_score_t> searching
	(
		!check && depth > 3 * PLY && moves.size() > 8 ?	tbb::filter::parallel : tbb::filter::serial_in_order,
		[&](move_t move)
		{
			...
			if (!test_check(succ, node.color()))
			{
				...
				if (!(legal && !check && !move.index && prune && mscore + scoreX <= best_score && !killer))
				{
					if (legal > 4 && depth > 3 * PLY && node[move.to] == no_piece && node[move.promotion] == 0 && !check && !move.index && !killer)
		//			if (legal > 4 && depth > 3 && mscore <= 0 && !check && !move.index)
					{
						const score_t score1 = -search(succ, -best_score - 1, -best_score, depth - 2 * PLY - (mscore < -1_P) * PLY / 2, height + 1, move);
					...
					{
					score = -search(succ, -best_score - 1, -best_score, depth - 1 * PLY /*+ ext*/, height + 1, move);
					if (!tbb::task::self().is_cancelled() && score > best_score && score < search_beta)
						score = -search(succ, -search_beta, -score, depth - 1 * PLY /*+ ext*/, height + 1, move);
					}
				}
			}
			return std::make_pair(move, score);
	...
	tbb::filter_t<move_score_t, void> saving
	(
		tbb::filter::serial_in_order,
		[&](move_score_t move_score)
		{
			const move_t move = move_score.first;
			const score_t score = move_score.second;
			if (score > best_score)
			{
				best_score = score;
				best_move = move;
				if (score >= search_beta)
				{
					...
					_t_table.put(node, move, search_beta, transposition_table_t::LOWER, depth);// + ext);
	...
	tbb::filter_t<void,void> pipeline = generating & searching & saving;
	tbb::parallel_pipeline(16, pipeline);
...
//			? 1_Q + bool(node.occupy<pawn_tag, white_tag>() & R7) * (1_Q - 1_P)
//			: 1_Q + bool(node.occupy<pawn_tag, black_tag>() & R2) * (1_Q - 1_P);
//}
...
//		if (node.score() * node.color() + moves.score(move) + 2_P < best_score)
//			break;
		const node_t succ(node, move);
		if (test_check(succ, node.color()))
			continue;
