...
template <typename moves_tag>
move_generator<moves_tag>::move_generator(const node_t& node)
:
...
template <typename moves_tag>
move_generator<moves_tag>::move_generator(const node_t& node, const history_table_t& h_table, const history_table_t& b_table, const killer_table_t::entry_t& k_entry1, const killer_table_t::entry_t& k_entry2)
:
	_end(generate(node, _moves.begin()))
{
	for (std::uint8_t index = 0; index < size(); ++index)
...
template <typename moves_tag>
moves_t::iterator
move_generator<moves_tag>::generate(const node_t& node, moves_t::iterator moves)
{
